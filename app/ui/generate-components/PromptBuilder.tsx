...
export default function PromptBuilder() {
  ...
  const handleTemplateClick = (templateData: PromptData) => {
    setPromptData(templateData);
    setGeneratedPrompt(''); // Clear generated prompt when a new template is selected
  };
          ...
          <Stack spacing={2}>
            {professionalTemplates.map((template) => (
              <Card key={template.title} variant="outlined">
                <CardActionArea onClick={() => handleTemplateClick(template.data)}>
                  <CardContent>
        ...
        <Grid item xs={12} md={9}>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={6}>
              <PromptField icon={<AccountCircle color="primary" />} label="Subject" badge="Core">
                <TextField name="subject" value={promptData.subject} onChange={handleInputChange} fullWidth multiline rows={3} variant="outlined" placeholder="Describe the main character or object..." />
              </PromptField>
            </Grid>
            <Grid item xs={12} md={6}>
              <PromptField icon={<Theaters color="primary" />} label="Context & Scene">
                <TextField name="context" value={promptData.context} onChange={handleInputChange} fullWidth multiline rows={3} variant="outlined" placeholder="Describe the environment, location, props..." />
              </PromptField>
            </Grid>
            <Grid item xs={12} md={6}>
              <PromptField icon={<DirectionsRun color="primary" />} label="Action & Movement">
                <TextField name="action" value={promptData.action} onChange={handleInputChange} fullWidth multiline rows={3} variant="outlined" placeholder="Describe specific actions, gestures..." />
              </PromptField>
            </Grid>
            <Grid item xs={12} md={6}>
              <PromptField icon={<Palette color="primary" />} label="Visual Style">
                <FormControl fullWidth variant="outlined">
                  <InputLabel id="visual-style-label">Choose visual style...</InputLabel>
                  <Select labelId="visual-style-label" id="visual-style-select" name="visualStyle" value={promptData.visualStyle} onChange={handleSelectChange} label="Choose visual style...">
            ...
            <Grid item xs={12} md={6}>
              <PromptField icon={<Videocam color="primary" />} label="Camera Movement">
                <FormControl fullWidth variant="outlined">
                  <InputLabel id="camera-movement-label">Choose camera movement...</InputLabel>
                  <Select labelId="camera-movement-label" id="camera-movement-select" name="cameraMovement" value={promptData.cameraMovement} onChange={handleSelectChange} label="Choose camera movement...">
            ...
            <Grid item xs={12} md={6}>
              <PromptField icon={<FilterCenterFocus color="primary" />} label="Composition">
                <TextField name="composition" value={promptData.composition} onChange={handleInputChange} fullWidth multiline rows={1} variant="outlined" placeholder="Describe framing, depth of field..." />
              </PromptField>
            </Grid>
            <Grid item xs={12} md={6}>
              <PromptField icon={<WbSunny color="primary" />} label="Lighting & Ambiance">
                <TextField name="lighting" value={promptData.lighting} onChange={handleInputChange} fullWidth multiline rows={1} variant="outlined" placeholder="Describe lighting, mood, and weather..." />
              </PromptField>
            </Grid>
            <Grid item xs={12} md={6}>
              <PromptField icon={<GraphicEq color="primary" />} label="Audio Description" badge="Important">
                <TextField name="audio" value={promptData.audio} onChange={handleInputChange} fullWidth multiline rows={3} variant="outlined" placeholder="Describe narration, sound effects, music..." />
              </PromptField>
            </Grid>
          </Grid>
