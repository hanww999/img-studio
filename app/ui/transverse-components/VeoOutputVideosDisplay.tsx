// 文件路径: app/ui/transverse-components/VeoOutputVideosDisplay.tsx (最终完整版)
...
const PromptDisplay = ({ prompt }: { prompt: string }) => {
  ...
  return (
    <>
      <Paper variant="outlined" sx={{ p: 1.5, mt: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
        <Typography variant="caption" sx={{ flexGrow: 1, maxHeight: 50, overflowY: 'auto' }}>{prompt}</Typography>
...
const EmptyState = () => {
  ...
  const exampleVideos: ExampleVideo[] = [
    ...
    { thumbnail: '/examples/video_thumb_1.jpg', videoSrc: '/examples/video_1.mp4', prompt: 'A dramatic cinematic shot of a futuristic car driving through a neon-lit city at night.' },
    ...
    { thumbnail: '/examples/video_thumb_2.jpg', videoSrc: '/examples/video_2.mp4', prompt: 'A beautiful drone shot flying over a tropical beach during sunset, hyperrealistic.' },
    ...
    <>
      ...
      {videoFullScreen && (
        <Modal open={!!videoFullScreen} onClose={() => setVideoFullScreen(null)} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Box sx={{ maxWidth: '80vw', maxHeight: '80vh', bgcolor: 'black' }}><video src={videoFullScreen.videoSrc} controls autoPlay style={{ width: '100%', height: '100%', maxHeight: '80vh' }} /></Box>
        </Modal>
      )}
    </>
  );
    ...
    <>
      <ImageList cols={generatedCount > 1 ? 2 : 1} gap={16} sx={{ m: 0, overflowY: 'auto' }}>
        {generatedVideosInGCS.map((video) => video.src ? (
          <Box key={video.key} sx={{ display: 'flex', flexDirection: 'column' }}>
            <ImageListItem sx={{ '&:hover .actions-bar': { opacity: 1 }, borderRadius: 3, overflow: 'hidden', position: 'relative' }}>
              <video src={video.src} width={video.width} height={video.height} style={{ width: '100%', height: 'auto', display: 'block', cursor: 'pointer' }} playsInline muted preload="metadata" onClick={() => setVideoFullScreen(video)} />
              <ImageListItemBar className="actions-bar" sx={{ background: 'linear-gradient(to top, rgba(0,0,0,0.8) 0%, transparent 100%)', opacity: 0, transition: 'opacity 0.3s ease' }} position="bottom"
                actionIcon={
                  <Stack direction="row" justifyContent="flex-end" gap={0.5} sx={{ p: 1, width: '100%' }}>
                    <Tooltip title="导出到媒体库"><IconButton size="small" sx={{ color: 'white' }} onClick={() => setVideoToExport(video)}><CreateNewFolderRounded /></IconButton></Tooltip>
                    <Tooltip title="下载"><IconButton size="small" sx={{ color: 'white' }} onClick={() => handleDLvideo(video)}>{isDLloading ? <CircularProgress size={20} color="inherit" /> : <Download />}</IconButton></Tooltip>
                  </Stack>
                }
              />
              <PlayArrowRounded sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', fontSize: '4rem', color: 'rgba(255, 255, 255, 0.9)', pointerEvents: 'none' }} />
            </ImageListItem>
            <PromptDisplay prompt={video.prompt} />
          </Box>
        ) : null)}
      </ImageList>
