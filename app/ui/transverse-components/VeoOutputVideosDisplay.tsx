// 文件路径: app/ui/transverse-components/VeoOutputVideosDisplay.tsx (最终确认版)
...
import { CreateNewFolderRounded, Download, PlayArrowRounded, Movie as MovieIcon, ChevronLeft, ChevronRight } from '@mui/icons-material'
import {
  Box, IconButton, Modal, Skeleton, ImageListItem, ImageList,
  ImageListItemBar, Stack, CircularProgress, Typography, Paper, Tooltip
} from '@mui/material'
  ...
  return (
    <>
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', textAlign: 'center', p: 3 }}>
        <MovieIcon sx={{ fontSize: 120, color: 'primary.main', mb: 2 }} />
        <Typography variant="h5" component="h2" sx={{ mt: 1, fontWeight: 'bold' }}>您的视频画廊</Typography>
        <Typography color="text.secondary" sx={{ mt: 1, mb: 4, maxWidth: '450px' }}>生成的视频将出现在这里。看看这些例子获取灵感吧！</Typography>
            ...
            <Stack direction="row" spacing={2} justifyContent="flex-start" sx={{ display: 'inline-flex', p: 1 }}>
              {exampleVideos.map((ex, index) => (
                <Tooltip title={ex.prompt} placement="top" arrow key={index}>
                  {/* [核心功能 2] 点击这个 Paper (缩略图) 就会触发放大播放 */}
    ...
    <>
      <ImageList cols={generatedCount > 1 ? 2 : 1} gap={16} sx={{ m: 0, overflowY: 'auto' }}>
        {generatedVideosInGCS.map((video) => video.src ? (
          <ImageListItem key={video.key} sx={{ '&:hover .actions-bar': { opacity: 1 }, borderRadius: 3, overflow: 'hidden', position: 'relative' }}>
