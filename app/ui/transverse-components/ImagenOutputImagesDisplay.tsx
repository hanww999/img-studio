    ...
    <>
      <ImageList cols={generatedCount > 1 ? 2 : 1} gap={16} sx={{ m: 0, overflowY: 'auto' }}>
        {generatedImagesInGCS.map((image) => (
          <Box key={image.key} sx={{ display: 'flex', flexDirection: 'column' }}>
            <ImageListItem sx={{ '&:hover .actions-bar': { opacity: 1 }, borderRadius: 3, overflow: 'hidden', position: 'relative' }}>
              <Image src={image.src} alt={image.altText} width={image.width} height={image.height} style={{ width: '100%', height: 'auto', display: 'block', cursor: 'pointer' }} placeholder="blur" blurDataURL={blurDataURL} quality={80} onClick={() => setImageFullScreen(image)} />
              <ImageListItemBar className="actions-bar" sx={{ background: 'linear-gradient(to top, rgba(0,0,0,0.8) 0%, transparent 100%)', opacity: 0, transition: 'opacity 0.3s ease' }} position="bottom"
                actionIcon={
                  <Stack direction="row" justifyContent="flex-end" gap={0.5} sx={{ p: 1, width: '100%' }}>
                    {isPromptReplayAvailable && !image.prompt.includes('[1]') && (<Tooltip title="More like this!"><IconButton size="small" sx={{ color: 'white' }} onClick={() => handleMoreLikeThisClick(image.prompt)}><Favorite /></IconButton></Tooltip>)}
                    {process.env.NEXT_PUBLIC_EDIT_ENABLED === 'true' && (<Tooltip title="Edit this image"><IconButton size="small" sx={{ color: 'white' }} onClick={() => handleEditClick(image.gcsUri)}><Edit /></IconButton></Tooltip>)}
                    {process.env.NEXT_PUBLIC_VEO_ENABLED === 'true' && process.env.NEXT_PUBLIC_VEO_ITV_ENABLED === 'true' && (<Tooltip title="Image to video"><IconButton size="small" sx={{ color: 'white' }} onClick={() => handleITVClick(image.gcsUri)}><VideocamRounded /></IconButton></Tooltip>)}
            ...
            <PromptDisplay prompt={image.prompt} />
          </Box>
        ))}
      </ImageList>
