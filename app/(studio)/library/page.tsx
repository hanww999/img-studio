...
export default function Page() {
    ...
    // FIX: Explicitly type the state to allow for 'null' or an object. This fixes the build error.
    ...
    // REFACTORED & FIXED: This single useEffect now handles both the initial load and subsequent filter changes.
    // It runs on mount (when filters is null, fetching all items) and whenever filters are updated.
    useEffect(() => {
        // When filters is null (initial load), it passes {}, fetching the first page.
        // When filters is an object, it passes the filters for a new search.
        // The 'true' for isReplacingExistingData ensures the view is reset on each new search.
    ...
    const handleDeletion = useCallback(async () => {
        if (deletionStatus === 'init') {
            ...
            try {
                ...
                else throw new Error('Deletion completed with an unknown status.')
            } catch (error: any) {
